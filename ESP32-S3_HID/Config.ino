...
// ESP32: EEPROM needs begin() before first use
static bool eepromReady = false;
static void ensureEEPROM() {
  if (!eepromReady) { EEPROM.begin(512); eepromReady = true; }
}
#endif  // USE_EEPROM
...
void setParam (u16 offset, u8 *addr_to, u8 size) {
  ...
  for (u8 i = 0; i < size; i++) {
    ensureEEPROM();
    uint8_t cur = EEPROM.read(offset + i);
    if (cur != addr_to[i]) {
      EEPROM.write(offset + i, addr_to[i]); // ESP32: update = read+compare+write
    }
  }
  EEPROM.commit(); // ESP32: flush to flash
#endif
}
...
void SetDefaultEEPROMConfig() { // milos - store default firmware settings in EEPROM
  ...
  // Defaults alinhados com profile1.txt "Hoverboard"
  v8 = 100;
  ...
  SetParam(PARAM_ADDR_SPR_GAIN, v8); // profile: SpringGain = 100
  v8 = 50;
...
#ifndef USE_AS5600
  v32 = 4096; // ESP32-S3 port: usa MT6701 via STM32 (CPR=4096). Original=2400 (600PPR optical)
#else
  v32 = 4096; // milos, default CPR value for 12bit magnetic encoder AS5600
#endif
  SetParam(PARAM_ADDR_ENC_CPR, v32); // milos, added
#ifndef USE_MCP4725
  v8 = 33; // profile1.txt: pwmstate=33=0b00100001 (Fast PWM, PWM+-, 8kHz)
#else
...
void LoadEEPROMConfig () { //milos, added - updates all v8 parameters from EEPROM
  ...
  // FIX: Sanitize pedal calibration values carregados do EEPROM.
  // Perfis gravados com Arduino original (ADC 10-bit, max=1023) ou calibração
  // incompleta (max=0, max=min, overflow) corrompem o range.
  // Se max ≤ 1023 e min < max: auto-escala 10-bit → 12-bit (×4).
  // Se inválido (max ≤ min, negativo, acima de maxCal): reset para range completo.
  auto fixCal = [](int16_t &lo, int16_t &hi, int16_t fullMax) {
    const int16_t tenBitMax = 1023;
    if (lo >= 0 && hi > lo && hi <= tenBitMax) {
      // Parece 10-bit (provavelmente perfil gravado com Arduino Pro Micro)
      lo = (int16_t)map(lo, 0, tenBitMax, 0, fullMax);
      hi = (int16_t)map(hi, 0, tenBitMax, 0, fullMax);
    } else if (hi <= lo || lo < 0 || hi > fullMax) {
      lo = 0; hi = fullMax;
    }
    // else: já está em 12-bit, usa como está
  };
  fixCal(accel.min,  accel.max,  (int16_t)maxCal);
  fixCal(clutch.min, clutch.max, (int16_t)maxCal);
  fixCal(hbrake.min, hbrake.max, (int16_t)maxCal);
  // brake é s32a (int32), tratamento separado
  {
    int16_t blo = (int16_t)brake.min, bhi = (int16_t)brake.max;
    fixCal(blo, bhi, (int16_t)maxCal);
    brake.min = blo; brake.max = bhi;
  }
#endif
}
