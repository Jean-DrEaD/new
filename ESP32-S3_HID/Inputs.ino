...
#include <Arduino.h>
#include <Wire.h>
#include "digitalWriteFast.h"  // ESP32 shim
...
void nextInputState() {
  ...
  if (SHIFTREG_STATE < 2) {
    ...
    if (SHIFTREG_STATE % 2 == 0) {
      bitVal = digitalRead(SHIFTREG_DATA_SW); // milos, faster reading (bit7 of PIND register is pin D6)
...
bool readSingleButton (uint8_t i) { // milos, added
  ...
  // FIX: Botões 0-3 são bits do campo HAT no descriptor HID.
  // No ESP32-S3 Super Mini, GPIO48 (LED RGB / WS2812) é mapeado como
  // "dummy sempre-HIGH", mas o driver do LED puxa a linha para LOW
  // na inicialização → !digitalRead(48) = 1 → botão fantasma disparado.
  // Solução: forçar 0 nos bits HAT — não há D-pad físico conectado.
  if (i < 4) return false;
#endif

  if (i == 0) {
    temp = !digitalRead(BUTTON0); // milos, read bit4 from PINF A3 (or bit4 from PIND D4 when no LC, or bit6 from PINC D5 on leonardo/micro when XY shifter) into buttons bit0
  } else if (i == 1) {
    temp = !digitalRead(BUTTON1); // milos, read bit1 from PINF A4 (or bit3 from PINB D14 on ProMicro) into buttons bit1
  } else if (i == 2) {
    temp = !digitalRead(BUTTON2); // milos, read bit0 from PINF A5 (or bit1 from PINB D15 on ProMicro) into buttons bit2
  } else if (i == 3) {
    temp = !digitalRead(BUTTON3); // milos, read bit6 from PIND D12 into buttons bit3
  } else if (i == 4) {
    temp = !digitalRead(BUTTON4); // milos, read bit7 from PIND D6 into buttons bit4
  } else if (i == 5) {
    temp = !digitalRead(BUTTON5); // milos, read bit6 from PINE D7 into buttons bit5
  } else if (i == 6) {
    temp = !digitalRead(BUTTON6); // milos, read bit4 from PINB D8 into buttons bit6
#ifndef USE_LOAD_CELL // milos, save some memory space if this button is unavailable
  } else if (i == 7) {
    temp = !digitalRead(BUTTON7); // milos, read bit6 from PINC D5 into buttons bit7
#endif // end of load cell
#ifdef USE_EXTRABTN // milos, if enabled we have 2 more extra digital buttons
  } else if (i == 8) {
    temp = !digitalRead(BUTTON8); // milos, read bit5 from PINF A2 into buttons bit8
  } else if (i == 9) {
    temp = !digitalRead(BUTTON9); // milos, read bit4 from PINF A3 into buttons bit9
#endif // end of extra button
  } else {
    temp = false;
  }
