...
#else
        // FIX: modo manual — resetar para range completo e salvar no EEPROM.
        // Original retornava 0 (não fazia nada) neste modo, impossibilitando
        // recuperação de calibração corrompida por perfis de Arduino (10-bit).
        accel.min  = 0;  accel.max  = maxCal;
        brake.min  = 0;  brake.max  = maxCal;
        clutch.min = 0;  clutch.max = maxCal;
        hbrake.min = 0;  hbrake.max = maxCal;
        // Persiste no EEPROM imediatamente
#ifdef USE_EEPROM
