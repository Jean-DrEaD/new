...
#include "USBCore.h"
#include "USBAPI.h"  // ESP32: Setup, USBDevice_, Joystick_
#include <stdint.h>
#include "debug.h"
...
b8 HID_GetReport (Setup& setup)
{
  ...
  if (report_id == 6)  // PID Block Load Feature
  {
    gNewEffectBlockLoad.reportId = 6;
...
b8 HID_SetReport (Setup& setup)
{
  u8 report_id = setup.wValueL;
  u8 report_type = setup.wValueH;
  if (report_id == 5)  // Create New Effect Feature
  {
...
// PID Status bit masks — defined here (before FfbInitPidState) so they are in scope
// for both FfbInitPidState() and FfbHandle_DeviceControl().
...
// FIX (Bug 2): Initialize pidState so ACTUATORS_ENABLED is set from power-on.
// Without this, pidState.status == 0 → Windows/ACC never sees the device as ready
// for FFB → no effects are ever sent → deadlock (ACC waits for ACTUATORS_ENABLED
// before sending DeviceControl, but DeviceControl was the only place that set it).
// Must be called from setup() AFTER FfbSetDriver().
void FfbInitPidState()
{
  pidState.reportId         = 2;
  pidState.status           = SAFETY_SWITCH | ACTUATOR_POWER | ACTUATORS_ENABLED;
  pidState.effectBlockIndex = 0;
}
...
volatile uint32_t gFfbRxCount       = 0;  // diagnostic: total FFB OUTPUT reports received
volatile bool     gFFB_autoCenter_prev = true;
...
void FfbOnUsbData(uint8_t *data, uint16_t len)
{
  ...
  // FIX: filter id=0x00 spam — means Windows doesn't recognize report IDs (descriptor issue).
  // With correct two-Application HID descriptor this should never appear.
  if (data[0] == 0x00) {
    static uint32_t _lastWarn = 0;
    if ((millis() - _lastWarn) > 2000) {
      _lastWarn = millis();
      Serial.printf("[FFB] WARN: report_id=0x00 — Windows not recognizing report IDs! Check HID descriptor.\n");
    }
    LEDs_SetAllLEDs(LEDS_NO_LEDS);
    return;
  }

  Serial.printf("[FFB] OUT id=0x%02X len=%u\n", data[0], len);
  gFfbRxCount++;
...
{
  // reportId is patched to match the actual descriptor ID in HID_GetReport
  outData->reportId = 6;  // will be overridden to 0x16 on ESP32 by HID_GetReport
...
void FfbHandle_DeviceControl(USB_FFBReport_DeviceControl_Output_Data_t *data)
{
  ...
  Bset(pidState.status, ACTUATORS_ENABLED);  // FIX: pre-enable actuators so games that skip DeviceControl still get FFB
  pidState.effectBlockIndex = 0;

  switch (control)
  {
    case 0x01:
      // HID PID spec: 0x01 = Enable Actuators
      LogTextLf("Enable Actuators");
      Bset(pidState.status, ACTUATORS_ENABLED);
      break;
    case 0x02:
      // HID PID spec: 0x02 = Disable Actuators
