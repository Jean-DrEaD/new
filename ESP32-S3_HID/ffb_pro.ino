...
// #include <util/delay.h>  // AVR-only: removido para ESP32
...
s32v cFFB::CalcTorqueCommands (s32v *pos) { // milos, pointer struct agument, returns struct, pos->x: x-axis, pos->y: y-axis
  ...
  command.x = s32(0);
  s32v pos_orig; pos_orig.x = (pos != NULL) ? pos->x : 0; // FIX: save original pos before endstop modifies it
  ...
  if (pos != NULL) { // milos, this check is always required for pointers
    ...
    if (gFFB.mAutoCenter) { // milos, desktop autocenter spring effect if no FFB from any app or game
      ...
      if (bitRead(effstate, 0)) {
        ...
        */
        ...
        // FIX: only process effects in PLAYING state.
        // Original code: Btest(ef.state, MEffectState_Allocated | MEffectState_Playing)
        // = ef.state & 0x03 → fires when EITHER bit is set, i.e. allocated-but-not-started
        // effects contribute force. Correct check: effect must have the Playing bit set.
    ...
    // FIX: endstop hard override.
    // When pos is beyond ROTATION_MID the center spring alone may have already
    // saturated the torque at its own value. This override guarantees the torque
    // is AT LEAST configStopGain% of max in the correct (returning) direction,
    // bypassing the clamp that was making the endstop feel soft.
    {
      s32 endstop_limit = (s32)ROTATION_MID;
      s32 min_stop_torq = (s32)MM_MAX_MOTOR_TORQUE * (s32)configStopGain / 100;
      if (pos_orig.x > endstop_limit) {
        if (command.x > -min_stop_torq) command.x = -min_stop_torq;
      } else if (pos_orig.x < -endstop_limit) {
        if (command.x < min_stop_torq) command.x = min_stop_torq;
      }
    }
