...
typedef struct {
  ...
  int32_t count_prev;
  int32_t pos_offset;      // subtracted from TotalCount so center = 0 (set by finalize_x_alignment)
  uint32_t align_cooldown_end;  // buzzerTimer value until which OC faults are still suppressed (post-alignment grace period)
  boolean_T applying_direction; // set during ApplyDirection() TIM4 reinit to freeze mechAngle in bldc.c
} SensorState;
#endif
...
/* Clear overcurrent fault flag and rearm DC-Link watchdog.
 * Call only when the overcurrent was expected/intentional (batente clamp, FFB spike).
 * Do NOT call during hardware shorts or genuine phase overcurrents. */
