...
SET_BIT(DBGMCU->CR,DBGMCU_CR_DBG_TIM8_STOP);
  ...
  // OCNPolarity NUNCA deve ser alterado condicionalmente por placa —
  // mudar esta linha sem oscilloscópio causa shoot-through e queima MOSFETs.
  // A diferença de direção GD32 vs STM32 é corrigida via INVERT_R_DIRECTION
  // condicional em config.h, sem tocar em nenhum parâmetro de hardware PWM.
