...
/* Forward declaration — defined later in this file, called before definition */
boolean_T encoder_alignment_active(void);
...
void HAL_ADC_LevelOutOfWindowCallback(ADC_HandleTypeDef *hadc) {
  ...
  if ((hadc != NULL) && (hadc == &hadc1 || hadc->Instance == hadc1.Instance)) {
    ...
    // FIX: Encoder alignment intentionally draws high current (phase 2 = 2x power).
    // Ignore overcurrent events during alignment to avoid permanently disabling the motor.
    if (!encoder_alignment_active()) {
      LEFT_TIM->BDTR &= ~TIM_BDTR_MOE;
      RIGHT_TIM->BDTR &= ~TIM_BDTR_MOE;
  ...
  if ((hadc != NULL) && (hadc == &hadc2 || hadc->Instance == hadc2.Instance)) {
    ...
    // FIX: Same as above for ADC2 (left DC link sensor).
    if (!encoder_alignment_active()) {
      LEFT_TIM->BDTR &= ~TIM_BDTR_MOE;
      RIGHT_TIM->BDTR &= ~TIM_BDTR_MOE;
      overcurrent_fault_flag = true;
...
/* Clear the overcurrent fault and rearm the DC-Link watchdog.
 * Safe to call from main.c without touching the static flag directly. */
void overcurrent_fault_clear(void) {
  overcurrent_fault_flag = false;
  #if defined(DC_LINK_WATCHDOG_ENABLE)
  if (dcLinkProtection) { DcLinkWatchdog_Init(); }
  #endif
}
...
boolean_T encoder_alignment_active(void) {
  ...
  return (encoder_x.align_state != 0U) || (encoder_y.align_state != 0U)
       || ((int32_t)(buzzerTimer - encoder_x.align_cooldown_end) < 0)
       || ((int32_t)(buzzerTimer - encoder_y.align_cooldown_end) < 0);
#elif defined(ENCODER_X)
...
void BLDC_Init(void) {
  ...
  rtP_Left.cf_currFilt          = CFG_CF_CURR_FILT;    // upstream 3d4a5b5: current measurement low-pass filter

  /* Encoder CPR / fractional-CPR fields: present only in BLDC_controller model 16.52+.
   * Guarded so ONE_AXIS variants (model 16.32, no a_cpr/a_fcpr fields) compile cleanly.
   * ENCODER_CPR is defined by config.h only when a variant sets ENCODER_X/Y without
   * overriding ENCODER_X_PPR (i.e. generic incremental encoder, not MT6701).
   * NOTE: FRAC_CPR must be defined alongside ENCODER_CPR in config.h for this path. */
  #if defined(ENCODER_CPR) && defined(FRAC_CPR)
  rtP_Left.a_cpr                = ENCODER_CPR;
  rtP_Left.a_fcpr               = FRAC_CPR;
  #endif
  /* When ENCODER_CPR is not defined (e.g. MT6701 with explicit ENCODER_X_PPR override)
   * or the model is 16.32 (no a_cpr/a_fcpr fields), no assignment is made — correct. */
  /* Feed-forward gain: present only in model 16.52+.
   * FF_GAIN is defined by config.h only for non-ONE_AXIS variants; guarded so
   * ONE_AXIS variants (model 16.32, no ff_gain field) compile without change. */
  #if defined(FF_GAIN)
...
static void Encoder_X_ApplyDirection(boolean_T forward_dir) {
  ...
  // GD32F103: TIM4 counts opposite to STM32 for the same physical rotation.
  // Compensate by inverting the forward_dir logic for IC2Polarity.
  // STM32: forward=true  → IC2=RISING,  forward=false → IC2=FALLING
  // GD32:  forward=true  → IC2=FALLING, forward=false → IC2=RISING
  #if defined(BOARD_GD32)
  encoder_config.IC2Polarity = forward_dir ? TIM_ICPOLARITY_FALLING : TIM_ICPOLARITY_RISING;
  #else
  ...
  // Freeze mechAngle in bldc.c ISR during TIM4 reinit to prevent spurious
  // mechAngle spikes (invalid CNT values during reinitialization cause FOC
  // overcurrent → audible buzz at end of alignment phase 2).
  encoder_x.applying_direction = 1;
  // Force full HAL reinit (including MspInit for GPIO) so IC polarity change takes effect.
  // Without this, HAL skips reinitialization when state is already READY.
  encoder_x_handle.State = HAL_TIM_STATE_RESET;
  HAL_TIM_Encoder_Init(&encoder_x_handle, &encoder_config);
  HAL_TIM_Encoder_Start(&encoder_x_handle, TIM_CHANNEL_ALL);
  encoder_x.applying_direction = 0;
}
...
void Encoder_X_Init(void) {
  ...
  encoder_x_handle.Init.AutoReloadPreload = TIM_AUTORELOAD_PRELOAD_DISABLE;
    ...
    // GD32F103: TIM4 encoder counts in opposite direction vs STM32F103 for the same
    // physical rotation. Inverting IC2Polarity compensates without swapping wires.
    #if defined(BOARD_GD32)
    encoder_config.IC2Polarity = TIM_ICPOLARITY_FALLING;
    #else
  ...
  if (HAL_TIM_Encoder_Init(&encoder_x_handle, &encoder_config) != HAL_OK) {
   ...
   if (HAL_TIM_Encoder_Start(&encoder_x_handle, TIM_CHANNEL_ALL) != HAL_OK) {
    ...
    memset(&encoder_x, 0, sizeof(SensorState));
    encoder_x.ini        = true;
    encoder_x.direction  = 1;
    encoder_x.count_prev = (int32_t)encoder_x_handle.Instance->CNT;
...
void Encoder_X_Align(void) {
 ...
 void handle_x_rotation_phase(uint32_t elapsed_ticks, uint32_t ramp_ms, uint32_t move_ms, uint32_t current_time) {
    ...
    } else if (elapsed_ticks < move_ms + T_MS(200)) {
        // Coast: manter inpTgt=0 por 200ms antes de iniciar fase 2.
        // O rotor ainda está em movimento no final do decel. Sem este período,
        // a fase 2 aplica 2×torque com o rotor em movimento → snap magnético.
        // 200ms garante que o rotor parou e a corrente residual decaiu.
        encoder_x.align_inpTgt = 0;
        // emulated_mech_count NÃO é alterado aqui: mantém o ângulo da última
        // posição da fase 1. Qualquer alteração de emulated_mech_count durante
        // o coast causa spike de mechAngle no ISR a 16kHz → zunido imediato.
    } else {
        // Transition to high-power phase. O snap de emulated_mech_count foi
        // removido: a fase 2 com 2×ALIGNMENT_X_POWER puxa o rotor para a posição
        // correta usando o mechAngle atual sem necessidade de ajuste prévio.
        encoder_x.offset = (ENCODER_X_CPR / N_POLE_PAIRS / 4); // 90° para eixo Q
 ...
 void handle_x_high_power_phase(uint32_t elapsed_ticks, uint32_t ramp_ms, uint32_t current_time) {
    if (elapsed_ticks < ramp_ms) {
        // Ramp to 1.5x power (era 2x: reduzi para diminuir pico de corrente no GD32/bancada)
        encoder_x.align_inpTgt = ((ALIGNMENT_X_POWER * 3 / 2) * elapsed_ticks) / ramp_ms;
    } else if (elapsed_ticks < T_MS(2000)) {
        // Hold at 1.5x power — suficiente para alinhar, 25% menos pico de corrente que 2x
        encoder_x.align_inpTgt = ALIGNMENT_X_POWER * 3 / 2;
    } else {
      int32_t seed_count;
      ...
      // BUG FIX (salto de posição + zunido GD32):
      // ANTES: seed_count era calculado a partir de emulated_mech_count (estimativa
      // em malha aberta). Com MT6701 (posição absoluta arbitrária no boot), o
      // emulated_mech_count difere do CNT real → seed_count errado → full_rotations=0
      // causa salto de TotalCount de vários CPR → Arduino vê "movimento" enorme →
      // reseta timer de auto-center → motor não centraliza.
      //
      // DEPOIS: usa o CNT real do TIM4. Após 2s de torque máximo, o rotor está
      // num zero elétrico real. Arredondamos para o múltiplo de (CPR/N_POLE_PAIRS)
      // mais próximo e aplicamos o offset de 90° (eixo Q).
      // full_rotations é recalculado para que TotalCount seja CONTÍNUO (sem salto).
      {
        int32_t real_cpr_period = (int32_t)(ENCODER_X_CPR / N_POLE_PAIRS); // ≈273 counts/ciclo elétrico
        int32_t real_cnt        = (int32_t)(encoder_x_handle.Instance->CNT);
        int32_t elec_pos        = real_cnt % real_cpr_period;
        // arredonda para zero elétrico (eixo D) mais próximo
        if (elec_pos > real_cpr_period / 2) elec_pos -= real_cpr_period;
        int32_t d_axis = real_cnt - elec_pos;
        // aplica offset +90° → eixo Q
        seed_count = normalize_x_encoder_count(d_axis + (int32_t)(ENCODER_X_CPR / N_POLE_PAIRS / 4));
      }

      // Restore correct FOC alignment: TIM4 = seed_count (Q-axis zero).
      // pos_offset is set to seed_count so get_x_TotalCount() returns 0 at center.
      // This fixes the "resistance every turn" bug WITHOUT breaking FOC torque.
      encoder_x.align_inpTgt = 0;
      ...
      encoder_x.pos_offset     = seed_count;  // center reference: TotalCount - pos_offset = 0
 ...
 void handle_x_move_back_phase(uint32_t elapsed_ticks, uint32_t ramp_ms, uint32_t move_ms, uint32_t current_time) {
    // Stage A: Ramp UP de 0 → ALIGNMENT_X_POWER.
    // inpTgt foi zerado antes de Encoder_X_ApplyDirection.
    // Subir de 0 evita salto brusco no 1º tick da fase 3.
    if (elapsed_ticks < ramp_ms) {
        encoder_x.align_inpTgt = (int16_t)((ALIGNMENT_X_POWER * (int32_t)elapsed_ticks) / (int32_t)ramp_ms);
    }
...
int32_t get_x_TotalCount(void){
  // Subtract pos_offset so the center position (where alignment finished) = 0.
  // pos_offset is set to TIM4 value at end of alignment (seed_count ≈ Q-axis).
  return encoder_x.full_rotations * ENCODER_X_CPR + encoder_x.count_prev - encoder_x.pos_offset;
}
 ...
 void finalize_x_alignment(void) {
       // MIN_MOVEMENT: minimum real encoder counts to confirm the motor physically responded.
       // Open-loop sweep simulates 2 elec cycles but rotor only partially follows due to
       // inertia/friction — observed real movement is 50-150 counts (old threshold 273 → always failed).
       // 1/6 of one electrical cycle (~45 counts / ~4° mechanical) is the detection floor.
       int32_t MIN_MOVMENT = (ENCODER_X_CPR / N_POLE_PAIRS) / 6;
       ...
       if (abs(movement) < MIN_MOVMENT){
        ...
        encoder_x.ali = true;
        overcurrent_fault_clear();        // clear any OC that fired during alignment
        encoder_x.align_cooldown_end = buzzerTimer + T_MS(500); // 500ms grace: suppress late OC triggers
        // Reset serial timeout: the counter saturates during the ~5s alignment sequence.
        // Without this reset, timeoutFlgSerial fires the instant alignment_active() returns false,
        // forcing ctrlModReq=OPEN_MODE and killing the motor immediately after alignment.
        ...
        #endif
        timeoutFlgSerial    = 0;
        // Recuperação do DC_LINK_WATCHDOG: se dcLinkProtection travou durante
        // uma parada brusca de FFB, o link serial válido restaura a operação.
        #if defined(DC_LINK_WATCHDOG_ENABLE)
        if (dcLinkProtection) { DcLinkWatchdog_Init(); }
        #endif
        rtP_Right.b_diagEna = DIAG_ENA;
        // Re-sync count tracking. TIM4 ≈ seed_count (motor stopped at Q-axis).
        // pos_offset = current TIM4 so get_x_TotalCount() = 0 at center.
        encoder_x.count_prev     = (int32_t)encoder_x_handle.Instance->CNT;
        encoder_x.ENCODER_COUNT  = encoder_x.count_prev;
        encoder_x.full_rotations = 0;
        encoder_x.pos_offset     = encoder_x.count_prev;  // TotalCount=0 at center
        encoder_x.align_state = 0;
       }    
}
#endif // ENCODER_X
...
static void Encoder_Y_ApplyDirection(boolean_T forward_dir) {
  ...
  // BUG FIX: freeze mechAngle in bldc.c ISR during TIM3 reinit to prevent spurious
  // mechAngle spikes (invalid CNT values during reinitialization cause FOC overcurrent).
  // Mirrors the same guard used in Encoder_X_ApplyDirection().
  // Without HAL_TIM_STATE_RESET, HAL skips reinitialization (state already READY)
  // → IC2 polarity change never takes effect → motor direction stays wrong after alignment.
  encoder_y.applying_direction = 1;
  encoder_y_handle.State = HAL_TIM_STATE_RESET;
  HAL_TIM_Encoder_Init(&encoder_y_handle, &encoder_config);
  HAL_TIM_Encoder_Start(&encoder_y_handle, TIM_CHANNEL_ALL);
  encoder_y.applying_direction = 0;
}
...
void Encoder_Y_Init(void) {
  ...
  if (HAL_TIM_Encoder_Init(&encoder_y_handle, &encoder_config) != HAL_OK) {
   ...
   if (HAL_TIM_Encoder_Start(&encoder_y_handle, TIM_CHANNEL_ALL) != HAL_OK) {
    ...
    // Initialize encoder state
    memset(&encoder_y, 0, sizeof(SensorState));
    encoder_y.ini        = true;
    encoder_y.direction  = 1;
    encoder_y.count_prev = (int32_t)encoder_y_handle.Instance->CNT;
...
void Encoder_Y_Align(void) {
 ...
 void handle_y_rotation_phase(uint32_t elapsed_ticks, uint32_t ramp_ms, uint32_t move_ms, uint32_t current_time) {
    ...
    encoder_y.emulated_mech_count = normalize_y_encoder_count(encoder_y.emulated_mech_count);
        ...
        // BUG FIX (Bug 5): coast 200ms antes de iniciar fase 2 (mesma lógica do eixo X).
        // O rotor ainda está em movimento no fim do decel. Sem este período, a fase 2
        // aplica 2×torque com rotor em movimento → snap magnético → zunido.
        encoder_y.align_inpTgt = 0;
        // emulated_mech_count NÃO é alterado: mantém o ângulo da última posição da fase 1.
    } else {
 ...
 void handle_y_high_power_phase(uint32_t elapsed_ticks, uint32_t ramp_ms, uint32_t current_time) {
    if (elapsed_ticks < ramp_ms) {
        // Ramp to 1.5x power (alinhado com eixo X)
        encoder_y.align_inpTgt = ((ALIGNMENT_Y_POWER * 3 / 2) * elapsed_ticks) / ramp_ms;
    } else if (elapsed_ticks < T_MS(1000)) {
        // Hold at 1.5x power
        encoder_y.align_inpTgt = ALIGNMENT_Y_POWER * 3 / 2;
  } else {
    // Record final position and move to next phase
      ...
      // BUG FIX (Bug 4 + mesma lógica do Bug 1 no eixo X):
      // Zero torque ANTES de ApplyDirection para evitar buzz no GD32.
      // seed_count usa CNT real (rotor está num zero elétrico após 1s de torque).
      encoder_y.align_inpTgt = 0;
      {
        int32_t real_cpr_period = (int32_t)(ENCODER_Y_CPR / N_POLE_PAIRS);
        int32_t real_cnt        = (int32_t)(encoder_y_handle.Instance->CNT);
        int32_t elec_pos        = real_cnt % real_cpr_period;
        if (elec_pos > real_cpr_period / 2) elec_pos -= real_cpr_period;
        int32_t d_axis = real_cnt - elec_pos;
        seed_count = normalize_y_encoder_count(d_axis + (int32_t)(ENCODER_Y_CPR / N_POLE_PAIRS / 4));
      }
      Encoder_Y_ApplyDirection(encoder_y.direction);
      __HAL_TIM_SET_COUNTER(&encoder_y_handle, seed_count);
      {
        int32_t old_total = get_y_TotalCount();
        encoder_y.count_prev    = seed_count;
        encoder_y.ENCODER_COUNT = seed_count;
        int32_t diff = old_total - seed_count;
        encoder_y.full_rotations = (int32_t)((float)diff / (float)ENCODER_Y_CPR + 0.5f);
        if (diff < 0) encoder_y.full_rotations = (int32_t)(((float)diff - 0.5f) / (float)ENCODER_Y_CPR);
      }
 ...
 void handle_y_move_back_phase(uint32_t elapsed_ticks, uint32_t ramp_ms, uint32_t move_ms, uint32_t current_time) {
    // Stage A: Ramp UP de 0 → ALIGNMENT_Y_POWER.
    // BUG FIX: o código anterior rampeava DE 2×ALIGNMENT_Y_POWER PARA ALIGNMENT_Y_POWER,
    // causando um salto brusco de torque (de 0 para 2× no primeiro tick da fase 3).
    // Isso ocorre porque align_inpTgt foi zerado em handle_y_high_power_phase antes de
    // chamar Encoder_Y_ApplyDirection — então a fase 3 começa com inpTgt=0, e o Stage A
    // antigo imediatamente pulava para 2×ALIGNMENT_Y_POWER. Mesma lógica do eixo X.
    if (elapsed_ticks < ramp_ms) {
        encoder_y.align_inpTgt = (int16_t)((ALIGNMENT_Y_POWER * (int32_t)elapsed_ticks) / (int32_t)ramp_ms);
    }
    // Stage B: Move emulated position back toward start
    else if (elapsed_ticks < move_ms) {
 ...
 void finalize_y_alignment(void) {
    // Calculate offset
       ...
       if (abs(movement) < MIN_MOVMENT){
        ...
        encoder_y.ali = true;
        // BUG FIX (Bug 3): resets críticos faltavam no eixo Y (presentes no X).
        // Sem eles: OC travado, serial timeout dispara OPEN_MODE imediatamente,
        // DC_LINK não recupera, diagEna só resetava Left (não Right).
        overcurrent_fault_clear();
        encoder_y.align_cooldown_end = buzzerTimer + T_MS(500);
        timeoutCntSerial_R  = 0;
        timeoutFlgSerial_R  = 0;
        #if defined(CONTROL_SERIAL_USART2) || defined(SIDEBOARD_SERIAL_USART2)
        timeoutCntSerial_L  = 0;
        ...
        rtP_Left.b_diagEna  = DIAG_ENA;
        rtP_Right.b_diagEna = DIAG_ENA;
        // Resync count tracking
        encoder_y.count_prev     = (int32_t)encoder_y_handle.Instance->CNT;
        encoder_y.ENCODER_COUNT  = encoder_y.count_prev;
...
void readInputRaw(void) {
  ...
  (void)idx; /* suppress unused-variable warning in ONE_AXIS variants */
  #ifdef CONTROL_ADC
  if (inIdx == CONTROL_ADC) {
...
void handleTimeout(void) {
  ...
  // Safe-state on timeout/fault — but ONLY after alignment completes (ali=true).
  // Without the && encoder_x.ali guard, any serial timeout during the ~5s alignment
  // window would permanently latch OPEN_MODE before the motor ever enters operation.

  // BUG FIX (overcurrent permanente durante operação FFB):
  // overcurrent_fault_flag é setado pelo ADC watchdog (hadc1/hadc2) quando a
  // corrente de fase ultrapassa DCL_HIGH_COUNTS. Durante operação normal com encoder
  // alinhado (ali=true), isso pode acontecer em picos de torque FFB (ex: spring force
  // forte, batente, impacto). O flag NUNCA era limpado exceto em finalize_x_alignment()
  // → primeiro overcurrent pós-boot travava o motor em OPEN_MODE para sempre.
  //
  // Recovery: se o encoder está alinhado, o serial ainda está vivo (Arduino ativo) e
  // não há outros faults reais (ADC, Gen, DLVPA), o overcurrent transitório é purgado.
  // O motor é reabilitado pelo main loop (enable=1) assim que input < 50.
  // A janela de 500ms (align_cooldown_end já usada pós-alinhamento) evita re-trigger
  // imediato caso o pico de corrente dure mais de um loop.
  // FIX: DLVPA recovery — DC link watchdog fires transiently during FFB regen/endstop.
  // If encoder is aligned and serial is alive, treat DLVPA as transient and rearm.
  // The braking resistor handles sustained overvoltage; this prevents permanent lockout.
  #if defined(DC_LINK_WATCHDOG_ENABLE)
  if (DLVPA() && encoder_x.ali && !encoder_alignment_active() &&
      !timeoutFlgSerial && !timeoutFlgADC && !timeoutFlgGen && !overcurrent_fault()) {
    DcLinkWatchdog_Init();  // clears dcLinkProtection and rearms watchdog
  }
  #endif

  if (overcurrent_fault() && encoder_x.ali && !encoder_alignment_active() &&
      !timeoutFlgSerial && !timeoutFlgADC && !timeoutFlgGen && !DLVPA()) {
    // Overcurrent transitório durante FFB normal: limpar e deixar o motor se recuperar.
    overcurrent_fault_clear();
  }

  if ((timeoutFlgADC || timeoutFlgSerial || timeoutFlgGen || DLVPA() || overcurrent_fault()) && (!encoder_alignment_active()) && encoder_x.ali) {
...
void poweroffPressCheck(void) {
    ...
    if (powerButtonPressed()) {
      ...
      if (cnt_press >= 5 * 100) {
        poweroff();          // pressão longa ≥5s: desliga
      } else if (cnt_press > 8) {
        enable = 1;          // pressão curta 80ms-5s: reabilita motor
      }
    }
  ...
  #else
    // ONE_AXIS_USART_VARIANT / DD wheel: comportamento semelhante ao non-hoverboard
    // mas com distinção curto/longo para evitar desligar acidentalmente ao religar.
    if (powerButtonPressed()) {
      uint16_t cnt_press = 0;
      enable = 0;                                             // desabilita motores durante o press
      while (powerButtonPressed()) {
        HAL_Delay(10);
        if (cnt_press++ == 5 * 100) { beepShort(5); }        // bip aos 5s avisa que vai desligar
      }
      if (cnt_press >= 5 * 100) {                            // pressão longa (≥5s): desliga
        poweroff();
      } else if (cnt_press > 8) {                            // pressão curta (80ms..5s): reabilita
        enable = 1;                                           // volta a operar sem desligar
      }
      // < 8 counts (~80ms): bounce, ignora
    }
  #endif
}
